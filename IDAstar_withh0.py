...
def ida_star_zero_heuristic(cost_matrix, start, goal):
    def heuristic_zero(node, visited_cities):
        return 0
    ...
    def dfs(current, g, bound, path, visited_cities):
        ...
        f = g + heuristic_zero(current, visited_cities)
        if f > bound:
            return f, path
        if current == goal:
...
for num_cities in num_cities_values:
    for seed in seeds:
        ...
        print("\nRunning IDA* with zero heuristic...")
        path, cost, run_time, expanded_nodes, generated_nodes = ida_star_zero_heuristic(cost_matrix, start_node, goal_node)
